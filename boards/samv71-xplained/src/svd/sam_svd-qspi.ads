...
--  character written in QSPI_TDR.TD has been transferred.
   ...
   type QSPI_MR_Register is record
      --  Serial Memory Mode
      SMM            : MR_SMMSelect := SAM_SVD.QSPI.SPI;
      --  Local Loopback Enable
      LLB            : MR_LLBSelect := SAM_SVD.QSPI.DISABLED;
      --  Wait Data Read Before Transfer
      WDRBT          : MR_WDRBTSelect := SAM_SVD.QSPI.DISABLED;
      --  Serial Memory Register Mode
      SMRM           : Boolean := False;
      --  Chip Select Mode
      CSMODE         : MR_CSMODESelect := SAM_SVD.QSPI.NOT_RELOADED;
      --  unspecified
      Reserved_6_7   : HAL.UInt2 := 16#0#;
      --  Number Of Bits Per Transfer
      NBBITS         : MR_NBBITSSelect := SAM_SVD.QSPI.Val_8_BIT;
      --  unspecified
...
--  performed.
   ...
   type QSPI_IFR_Register is record
      --  Width of Instruction Code, Address, Option Code and Data
      WIDTH          : IFR_WIDTHSelect := SAM_SVD.QSPI.SINGLE_BIT_SPI;
      --  unspecified
      ...
      --  Option Code Length
      OPTL           : IFR_OPTLSelect := SAM_SVD.QSPI.OPTION_1BIT;
      --  Address Length
      ADDRL          : IFR_ADDRLSelect := SAM_SVD.QSPI.Val_24_BIT;
      --  unspecified
      Reserved_11_11 : HAL.Bit := 16#0#;
      --  Data Transfer Type
      TFRTYP         : IFR_TFRTYPSelect := SAM_SVD.QSPI.TRSFR_READ;
      --  Continuous Read Mode
      CRM            : IFR_CRMSelect := SAM_SVD.QSPI.DISABLED;
      --  unspecified
   ...
   type QSPI_SMR_Register is record
      --  Scrambling/Unscrambling Enable
      SCREN         : SMR_SCRENSelect := SAM_SVD.QSPI.DISABLED;
