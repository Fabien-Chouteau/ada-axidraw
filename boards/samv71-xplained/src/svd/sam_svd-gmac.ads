...
package SAM_SVD.GMAC is
   ...
   type GMAC_GMAC_NCFGR_Register is record
      ...
      RFCS           : Boolean := False;
      --  MDC CLock Division
      CLK            : GMAC_NCFGR_CLKSelect := SAM_SVD.GMAC.MCK_8;
      --  Data Bus Width
   ...
   type GMAC_DCFGR_FBLDOSelect is
     (--  Reset value for the field
      GMAC_DCFGR_FBLDOSelect_Reset,
      ...
      INCR16)
     with Size => 5;
   for GMAC_DCFGR_FBLDOSelect use
     (GMAC_DCFGR_FBLDOSelect_Reset => 0,
      SINGLE => 1,
      INCR4 => 4,
      INCR8 => 8,
      INCR16 => 16);
   ...
   type GMAC_DCFGR_RXBMSSelect is
     (--  4/8 Kbyte Memory Size
      ...
      FULL)
     with Size => 2;
   for GMAC_DCFGR_RXBMSSelect use
     (EIGHTH => 0,
      QUARTER => 1,
      HALF => 2,
      FULL => 3);
   ...
   type GMAC_GMAC_DCFGR_Register is record
      --  Fixed Burst Length for DMA Data Operations:
      FBLDO          : GMAC_DCFGR_FBLDOSelect := GMAC_DCFGR_FBLDOSelect_Reset;
      --  unspecified
      ...
      ESPA           : Boolean := False;
      --  Receiver Packet Buffer Memory Size Select
      RXBMS          : GMAC_DCFGR_RXBMSSelect := SAM_SVD.GMAC.EIGHTH;
   ...
   type GMAC_ST2CW10_OFFSSTRTSelect is
     (--  Offset from the start of the frame
      ...
      TCP_UDP)
     with Size => 2;
   for GMAC_ST2CW10_OFFSSTRTSelect use
     (FRAMESTART => 0,
      ETHERTYPE => 1,
      IP => 2,
      TCP_UDP => 3);
   ...
   type GMAC_ST2CW_Register_1 is record
      ...
      --  Ethernet Frame Offset Start
      OFFSSTRT      : GMAC_ST2CW10_OFFSSTRTSelect := SAM_SVD.GMAC.FRAMESTART;
      --  unspecified
      Reserved_9_31 : HAL.UInt23 := 16#0#;
   end record
     with Volatile_Full_Access, Object_Size => 32,
