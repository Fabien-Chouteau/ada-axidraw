...
package SAM_SVD.HSMCI is
   ...
   type HSMCI_DTOR_DTOMULSelect is
     (--  DTOCYC
      Val_1,
      --  DTOCYC x 16
      Val_16,
      ...
      Val_1048576)
     with Size => 3;
   for HSMCI_DTOR_DTOMULSelect use
     (Val_1 => 0,
      Val_16 => 1,
      Val_128 => 2,
      Val_256 => 3,
   ...
   type HSMCI_HSMCI_DTOR_Register is record
      ...
      --  Data Timeout Multiplier
      DTOMUL        : HSMCI_DTOR_DTOMULSelect := SAM_SVD.HSMCI.Val_1;
      --  unspecified
   ...
   --  SDCard/SDIO Slot
   type HSMCI_SDCR_SDCSELSelect is
     (--  Slot A is selected.
      SLOTA)
     with Size => 2;
   for HSMCI_SDCR_SDCSELSelect use
     (SLOTA => 0);

   --  SDCard/SDIO Bus Width
   type HSMCI_SDCR_SDCBUSSelect is
     (--  1 bit
      Val_1,
      --  4 bits
      Val_4,
      ...
      Val_8)
     with Size => 2;
   for HSMCI_SDCR_SDCBUSSelect use
     (Val_1 => 0,
      Val_4 => 2,
      Val_8 => 3);
   ...
   type HSMCI_HSMCI_SDCR_Register is record
      --  SDCard/SDIO Slot
      SDCSEL        : HSMCI_SDCR_SDCSELSelect := SAM_SVD.HSMCI.SLOTA;
      --  unspecified
      Reserved_2_5  : HAL.UInt4 := 16#0#;
      --  SDCard/SDIO Bus Width
      SDCBUS        : HSMCI_SDCR_SDCBUSSelect := SAM_SVD.HSMCI.Val_1;
      --  unspecified
      Reserved_8_31 : HAL.UInt24 := 16#0#;
   end record
     with Volatile_Full_Access, Object_Size => 32,
   ...
   --  Response Type
   type HSMCI_CMDR_RSPTYPSelect is
     (--  No response
      NORESP,
      --  48-bit response
      Val_48_BIT,
      ...
      R1B)
     with Size => 2;
   for HSMCI_CMDR_RSPTYPSelect use
     (NORESP => 0,
      Val_48_BIT => 1,
      Val_136_BIT => 2,
      R1B => 3);
   ...
   --  Special Command
   type HSMCI_CMDR_SPCMDSelect is
     (--  Not a special CMD.
      STD,
...
--  boot operation mode.
      EBO)
     with Size => 3;
   for HSMCI_CMDR_SPCMDSelect use
     (STD => 0,
      INIT => 1,
      SYNC => 2,
      CE_ATA => 3,
   ...
   --  Open Drain Command
   type HSMCI_CMDR_OPDCMDSelect is
     (--  Push pull command.
      PUSHPULL,
      --  Open drain command.
      OPENDRAIN)
     with Size => 1;
   for HSMCI_CMDR_OPDCMDSelect use
     (PUSHPULL => 0,
      OPENDRAIN => 1);

   --  Max Latency for Command to Response
   type HSMCI_CMDR_MAXLATSelect is
     (--  5-cycle max latency.
      Val_5,
      --  64-cycle max latency.
      Val_64)
     with Size => 1;
   for HSMCI_CMDR_MAXLATSelect use
     (Val_5 => 0,
      Val_64 => 1);

   --  Transfer Command
   type HSMCI_CMDR_TRCMDSelect is
     (--  No data transfer
      NO_DATA,
      --  Start data transfer
      START_DATA,
      ...
      STOP_DATA)
     with Size => 2;
   for HSMCI_CMDR_TRCMDSelect use
     (NO_DATA => 0,
      START_DATA => 1,
      STOP_DATA => 2);

   --  Transfer Direction
   type HSMCI_CMDR_TRDIRSelect is
     (--  Write.
      WRITE,
      --  Read.
      READ)
     with Size => 1;
   for HSMCI_CMDR_TRDIRSelect use
     (WRITE => 0,
      READ => 1);

   --  Transfer Type
   type HSMCI_CMDR_TRTYPSelect is
     (--  MMC/SD Card Single Block
      ...
      BLOCK)
     with Size => 3;
   for HSMCI_CMDR_TRTYPSelect use
     (SINGLE => 0,
      MULTIPLE => 1,
      STREAM => 2,
      BYTE => 4,
   ...
   type HSMCI_CMDR_IOSPCMDSelect is
     (--  Not an SDIO Special Command
      ...
      RESUME)
     with Size => 2;
   for HSMCI_CMDR_IOSPCMDSelect use
     (STD => 0,
      SUSPEND => 1,
      RESUME => 2);

   --  ATA with Command Completion Signal
   type HSMCI_CMDR_ATACSSelect is
...
--  amount of time (HSMCI_CSTOR).
      COMPLETION)
     with Size => 1;
   for HSMCI_CMDR_ATACSSelect use
     (NORMAL => 0,
      COMPLETION => 1);
   ...
   type HSMCI_HSMCI_CMDR_Register is record
      ...
      RSPTYP         : HSMCI_CMDR_RSPTYPSelect := SAM_SVD.HSMCI.NORESP;
      --  Write-only. Special Command
      SPCMD          : HSMCI_CMDR_SPCMDSelect := SAM_SVD.HSMCI.STD;
      --  Write-only. Open Drain Command
      OPDCMD         : HSMCI_CMDR_OPDCMDSelect := SAM_SVD.HSMCI.PUSHPULL;
      --  Write-only. Max Latency for Command to Response
      MAXLAT         : HSMCI_CMDR_MAXLATSelect := SAM_SVD.HSMCI.Val_5;
      --  unspecified
      Reserved_13_15 : HAL.UInt3 := 16#0#;
      --  Write-only. Transfer Command
      TRCMD          : HSMCI_CMDR_TRCMDSelect := SAM_SVD.HSMCI.NO_DATA;
      --  Write-only. Transfer Direction
      TRDIR          : HSMCI_CMDR_TRDIRSelect := SAM_SVD.HSMCI.WRITE;
      --  Write-only. Transfer Type
      TRTYP          : HSMCI_CMDR_TRTYPSelect := SAM_SVD.HSMCI.SINGLE;
      --  unspecified
      Reserved_22_23 : HAL.UInt2 := 16#0#;
      --  Write-only. SDIO Special Command
      IOSPCMD        : HSMCI_CMDR_IOSPCMDSelect := SAM_SVD.HSMCI.STD;
      --  Write-only. ATA with Command Completion Signal
      ATACS          : HSMCI_CMDR_ATACSSelect := SAM_SVD.HSMCI.NORMAL;
   ...
   type HSMCI_CSTOR_CSTOMULSelect is
     (--  CSTOCYC x 1
      Val_1,
      --  CSTOCYC x 16
      Val_16,
      ...
      Val_1048576)
     with Size => 3;
   for HSMCI_CSTOR_CSTOMULSelect use
     (Val_1 => 0,
      Val_16 => 1,
      Val_128 => 2,
      Val_256 => 3,
   ...
   type HSMCI_HSMCI_CSTOR_Register is record
      ...
      --  Completion Signal Timeout Multiplier
      CSTOMUL       : HSMCI_CSTOR_CSTOMULSelect := SAM_SVD.HSMCI.Val_1;
      --  unspecified
      Reserved_7_31 : HAL.UInt25 := 16#0#;
   end record
     with Volatile_Full_Access, Object_Size => 32,
   ...
   type HSMCI_DMA_CHKSIZESelect is
     (--  1 data available
      Val_1,
      --  2 data available
      Val_2,
      ...
      Val_16)
     with Size => 3;
   for HSMCI_DMA_CHKSIZESelect use
     (Val_1 => 0,
      Val_2 => 1,
      Val_4 => 2,
      Val_8 => 3,
   ...
   type HSMCI_HSMCI_DMA_Register is record
      ...
      --  DMA Channel Read and Write Chunk Size
      CHKSIZE       : HSMCI_DMA_CHKSIZESelect := SAM_SVD.HSMCI.Val_1;
      --  unspecified
   ...
   --  Write Protect Key
   type HSMCI_WPMR_WPKEYSelect is
     (--  Reset value for the field
      HSMCI_WPMR_WPKEYSelect_Reset,
      --  Writing any other value in this field aborts the write operation of the
--  WPEN bit.Always reads as 0.
      PASSWD)
     with Size => 24;
   for HSMCI_WPMR_WPKEYSelect use
     (HSMCI_WPMR_WPKEYSelect_Reset => 0,
      PASSWD => 5063497);
   ...
   type HSMCI_HSMCI_WPMR_Register is record
      ...
      --  Write Protect Key
      WPKEY        : HSMCI_WPMR_WPKEYSelect := HSMCI_WPMR_WPKEYSelect_Reset;
   end record
     with Volatile_Full_Access, Object_Size => 32,
          Bit_Order => System.Low_Order_First;
