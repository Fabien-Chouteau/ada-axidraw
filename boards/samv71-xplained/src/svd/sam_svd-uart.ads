...
--  (2 over 3 majority).
      ENABLED)
     with Size => 1;
   for UART_MR_FILTERSelect use
     (DISABLED => 0,
      ENABLED => 1);

   --  Parity Type
   type UART_MR_PARSelect is
     (--  Even Parity
      EVEN,
      --  Odd Parity
      ODD,
...
--  Management Controller (PMC)).
      PMC_PCK)
     with Size => 1;
   for UART_MR_BRSRCCKSelect use
     (PERIPH_CLK => 0,
      PMC_PCK => 1);

   --  Channel Mode
   type UART_MR_CHMODESelect is
     (--  Normal mode
      NORMAL,
      --  Automatic echo
      AUTOMATIC,
   ...
   type UART_UART_MR_Register is record
      ...
      --  Receiver Digital Filter
      FILTER         : UART_MR_FILTERSelect := SAM_SVD.UART.DISABLED;
      --  unspecified
      Reserved_5_8   : HAL.UInt4 := 16#0#;
      --  Parity Type
      PAR            : UART_MR_PARSelect := SAM_SVD.UART.EVEN;
      --  Baud Rate Source Clock
      BRSRCCK        : UART_MR_BRSRCCKSelect := SAM_SVD.UART.PERIPH_CLK;
      --  unspecified
      Reserved_13_13 : HAL.Bit := 16#0#;
      --  Channel Mode
      CHMODE         : UART_MR_CHMODESelect := SAM_SVD.UART.NORMAL;
      --  unspecified
      Reserved_16_31 : HAL.UInt16 := 16#0#;
   end record
     with Volatile_Full_Access, Object_Size => 32,
   ...
   type UART_UART_CMPR_Register is record
      ...
      --  Comparison Mode
      CMPMODE        : UART_CMPR_CMPMODESelect := SAM_SVD.UART.FLAG_ONLY;
      --  unspecified
      Reserved_13_13 : HAL.Bit := 16#0#;
      --  Compare Parity
   ...
   type UART_WPMR_WPKEYSelect is
     (--  Reset value for the field
      UART_WPMR_WPKEYSelect_Reset,
      --  Writing any other value in this field aborts the write operation.Always
--  reads as 0.
      PASSWD)
     with Size => 24;
   for UART_WPMR_WPKEYSelect use
     (UART_WPMR_WPKEYSelect_Reset => 0,
      PASSWD => 5587282);
   ...
   type UART_UART_WPMR_Register is record
      ...
      --  Write Protection Key
      WPKEY        : UART_WPMR_WPKEYSelect := UART_WPMR_WPKEYSelect_Reset;
   end record
     with Volatile_Full_Access, Object_Size => 32,
          Bit_Order => System.Low_Order_First;
