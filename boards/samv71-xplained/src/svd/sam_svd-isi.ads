...
package SAM_SVD.ISI is
   ...
   type ISI_ISI_CFG1_Register is record
      ...
      FULL           : Boolean := False;
      --  Threshold Mask
      THMASK         : ISI_CFG1_THMASKSelect := SAM_SVD.ISI.BEATS_4;
      --  unspecified
   ...
   type ISI_CFG2_YCC_SWAPSelect is
     (--  Byte 0 Cb(i)Byte 1 Y(i)Byte 2 Cr(i)Byte 3 Y(i+1)
      ...
      MODE3)
     with Size => 2;
   for ISI_CFG2_YCC_SWAPSelect use
     (DEFAULT => 0,
      MODE1 => 1,
      MODE2 => 2,
      MODE3 => 3);
   ...
   type ISI_CFG2_RGB_CFGSelect is
     (--  Byte 0 R/G(MSB)Byte 1 G(LSB)/BByte 2 R/G(MSB)Byte 3 G(LSB)/B
      ...
      MODE3)
     with Size => 2;
   for ISI_CFG2_RGB_CFGSelect use
     (DEFAULT => 0,
      MODE1 => 1,
      MODE2 => 2,
      MODE3 => 3);
   ...
   type ISI_ISI_CFG2_Register is record
      ...
      --  YCrCb Format Swap Mode
      YCC_SWAP       : ISI_CFG2_YCC_SWAPSelect := SAM_SVD.ISI.DEFAULT;
      --  RGB Pixel Mapping Configuration
      RGB_CFG        : ISI_CFG2_RGB_CFGSelect := SAM_SVD.ISI.DEFAULT;
   end record
     with Volatile_Full_Access, Object_Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   type ISI_WPMR_WPKEYSelect is
     (--  Reset value for the field
      ISI_WPMR_WPKEYSelect_Reset,
      --  Writing any other value in this field aborts the write operation of the
--  WPEN bit.Always reads as 0.
      PASSWD)
     with Size => 24;
   for ISI_WPMR_WPKEYSelect use
     (ISI_WPMR_WPKEYSelect_Reset => 0,
      PASSWD => 4805449);
   ...
   type ISI_ISI_WPMR_Register is record
      ...
      --  Write Protection Key Password
      WPKEY        : ISI_WPMR_WPKEYSelect := ISI_WPMR_WPKEYSelect_Reset;
   end record
     with Volatile_Full_Access, Object_Size => 32,
          Bit_Order => System.Low_Order_First;
