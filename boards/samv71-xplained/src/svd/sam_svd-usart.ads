...
package SAM_SVD.USART is
   ...
   type US_MR_USART_MODESelect is
     (--  Normal mode
      NORMAL,
      --  RS485
      RS485,
      ...
      SPI_SLAVE)
     with Size => 4;
   for US_MR_USART_MODESelect use
     (NORMAL => 0,
      RS485 => 1,
      HW_HANDSHAKING => 2,
      MODEM => 3,
   ...
   --  Clock Selection
   type US_MR_USCLKSSelect is
     (--  Peripheral clock is selected
      MCK,
      --  Peripheral clock divided (DIV=DIV=8) is selected
      DIV,
      --  PMC programmable clock (PCK) is selected. If the SCK pin is driven (CLKO =
--  1), the CD field must be greater than 1.
      PCK,
      --  Serial clock (SCK) is selected
      SCK)
     with Size => 2;
   for US_MR_USCLKSSelect use
     (MCK => 0,
      DIV => 1,
      PCK => 2,
      SCK => 3);
   ...
   type US_MR_NBSTOPSelect is
     (--  1 stop bit
      ...
      Val_2_BIT)
     with Size => 2;
   for US_MR_NBSTOPSelect use
     (Val_1_BIT => 0,
      Val_1_5_BIT => 1,
      Val_2_BIT => 2);

   --  Channel Mode
   type US_MR_CHMODESelect is
     (--  Normal mode
      NORMAL,
   ...
   type USART_US_MR_Register is record
      --  USART Mode of Operation
      USART_MODE     : US_MR_USART_MODESelect := SAM_SVD.USART.NORMAL;
      --  Clock Selection
      USCLKS         : US_MR_USCLKSSelect := SAM_SVD.USART.MCK;
      --  Character Length
      CHRL           : US_MR_CHRLSelect := SAM_SVD.USART.Val_5_BIT;
      --  Synchronous Mode Select
      SYNC           : Boolean := False;
      --  Parity Type
      PAR            : US_MR_PARSelect := SAM_SVD.USART.EVEN;
      --  Number of Stop Bits
      NBSTOP         : US_MR_NBSTOPSelect := SAM_SVD.USART.Val_1_BIT;
      --  Channel Mode
      CHMODE         : US_MR_CHMODESelect := SAM_SVD.USART.NORMAL;
      --  Bit Order
      MSBF           : Boolean := False;
   ...
   type USART_US_MAN_Register is record
      ...
      --  Transmitter Preamble Pattern
      TX_PP          : US_MAN_TX_PPSelect := SAM_SVD.USART.ALL_ONE;
      --  unspecified
      ...
      Reserved_20_23 : HAL.UInt4 := 16#0#;
      --  Receiver Preamble Pattern detected
      RX_PP          : US_MAN_RX_PPSelect := SAM_SVD.USART.ALL_ONE;
      --  unspecified
   ...
   type USART_US_LINMR_Register is record
      --  LIN Node Action
      NACT           : US_LINMR_NACTSelect := SAM_SVD.USART.PUBLISH;
      --  Parity Disable
   ...
   type US_WPMR_WPKEYSelect is
     (--  Reset value for the field
      US_WPMR_WPKEYSelect_Reset,
      --  Writing any other value in this field aborts the write operation of the
--  WPEN bit. Always reads as 0.
      PASSWD)
     with Size => 24;
   for US_WPMR_WPKEYSelect use
     (US_WPMR_WPKEYSelect_Reset => 0,
      PASSWD => 5591873);
   ...
   type USART_US_WPMR_Register is record
      ...
      --  Write Protection Key
      WPKEY        : US_WPMR_WPKEYSelect := US_WPMR_WPKEYSelect_Reset;
   end record
     with Volatile_Full_Access, Object_Size => 32,
          Bit_Order => System.Low_Order_First;
