...
package SAM_SVD.MLB is
   ...
   type MLB_MLBC0_MLBCLKSelect is
     (--  256xFs (for MLBPEN = 0)
      Val_256_FS,
      --  512xFs (for MLBPEN = 0)
      Val_512_FS,
   ...
   type MLB_MLB_MLBC0_Register is record
      ...
      --  MLBCLK (MediaLB clock) speed select
      MLBCLK         : MLB_MLBC0_MLBCLKSelect := SAM_SVD.MLB.Val_256_FS;
      ...
      --  The number of frames per sub-buffer for synchronous channels
      FCNT           : MLB_MLBC0_FCNTSelect := SAM_SVD.MLB.Val_1_FRAME;
      --  unspecified
   ...
   --  Target Location Bit
   type MLB_MADR_TBSelect is
     (--  Selects CTR
      CTR,
      --  Selects DBR
      DBR)
     with Size => 1;
   for MLB_MADR_TBSelect use
     (CTR => 0,
      DBR => 1);
   ...
   type MLB_MLB_MADR_Register is record
      ...
      --  Target Location Bit
      TB             : MLB_MADR_TBSelect := SAM_SVD.MLB.CTR;
   ...
   type MLB_ACTL_MPBSelect is
     (--  Single-packet mode
      SINGLE_PACKET,
      --  Multiple-packet mode
      MULTIPLE_PACKET)
     with Size => 1;
   for MLB_ACTL_MPBSelect use
     (SINGLE_PACKET => 0,
      MULTIPLE_PACKET => 1);
   ...
   type MLB_MLB_ACTL_Register is record
      ...
      --  DMA Packet Buffering Mode
      MPB           : MLB_ACTL_MPBSelect := SAM_SVD.MLB.SINGLE_PACKET;
      --  unspecified
