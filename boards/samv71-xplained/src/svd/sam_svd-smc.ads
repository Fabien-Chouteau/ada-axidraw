...
package SAM_SVD.SMC is
   ...
   --  NWAIT Mode
   type SMC_MODE_EXNW_MODESelect is
...
--  access is extended until NWAIT returns high.
      READY)
     with Size => 2;
   for SMC_MODE_EXNW_MODESelect use
     (DISABLED => 0,
      FROZEN => 2,
      READY => 3);

   --  Byte Access Type
   type SMC_MODE_BATSelect is
     (--  Byte select access type:- Write operation is controlled using NCS, NWE,
--  NBS0, NBS1.- Read operation is controlled using NCS, NRD, NBS0, NBS1.
      BYTE_SELECT,
      --  Byte write access type:- Write operation is controlled using NCS, NWR0,
--  NWR1.- Read operation is controlled using NCS and NRD.
      BYTE_WRITE)
     with Size => 1;
   for SMC_MODE_BATSelect use
     (BYTE_SELECT => 0,
      BYTE_WRITE => 1);

   --  Data Bus Width
   type SMC_MODE_DBWSelect is
     (--  8-bit Data Bus
      Val_8_BIT,
      --  16-bit Data Bus
      Val_16_BIT)
     with Size => 1;
   for SMC_MODE_DBWSelect use
     (Val_8_BIT => 0,
      Val_16_BIT => 1);
   ...
   type SMC_SMC_MODE_SMC_SMC_CS_NUMBER_Register is record
      ...
      --  NWAIT Mode
      EXNW_MODE      : SMC_MODE_EXNW_MODESelect := SAM_SVD.SMC.DISABLED;
      --  unspecified
      Reserved_6_7   : HAL.UInt2 := 16#0#;
      --  Byte Access Type
      BAT            : SMC_MODE_BATSelect := SAM_SVD.SMC.BYTE_SELECT;
      --  unspecified
      Reserved_9_11  : HAL.UInt3 := 16#0#;
      --  Data Bus Width
      DBW            : SMC_MODE_DBWSelect := SAM_SVD.SMC.Val_8_BIT;
      --  unspecified
      ...
      --  Page Size
      PS             : SMC_MODE_PSSelect := SAM_SVD.SMC.Val_4_BYTE;
      --  unspecified
      Reserved_30_31 : HAL.UInt2 := 16#0#;
   end record
     with Volatile_Full_Access, Object_Size => 32,
   ...
   type SMC_WPMR_WPKEYSelect is
     (--  Reset value for the field
      SMC_WPMR_WPKEYSelect_Reset,
      --  Writing any other value in this field aborts the write operation of the
--  WPEN bit. Always reads as 0.
      PASSWD)
     with Size => 24;
   for SMC_WPMR_WPKEYSelect use
     (SMC_WPMR_WPKEYSelect_Reset => 0,
      PASSWD => 5459267);
   ...
   type SMC_SMC_WPMR_Register is record
      ...
      Reserved_1_7 : HAL.UInt7 := 16#0#;
      --  Write Protection Key
      WPKEY        : SMC_WPMR_WPKEYSelect := SMC_WPMR_WPKEYSelect_Reset;
   end record
     with Volatile_Full_Access, Object_Size => 32,
          Bit_Order => System.Low_Order_First;
