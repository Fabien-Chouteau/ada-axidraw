...
package Settings is
   ...
   Step_Per_Millimeter : constant Gcode.Float_Position := (80.0, 80.0, 250.0);
   ...
   Stepper_Min_Frequency : constant Gcode.Frequency_Value := 3_000.0;
   ...
   Tool_Max_Limit_Cartesian : constant Gcode.Step_Position :=
     (X_Axis => Integer (Step_Per_Millimeter (X_Axis)) * 300,
      Y_Axis => Integer (Step_Per_Millimeter (Y_Axis)) * 000,
      Z_Axis => Integer (Step_Per_Millimeter (Z_Axis)) * 001);
   --  Tool max position in carstesian coordinate

   Tool_Min_Limit_Cartesian : constant Gcode.Step_Position :=
     (X_Axis => Integer (Step_Per_Millimeter (X_Axis)) *   000,
      Y_Axis => Integer (Step_Per_Millimeter (Y_Axis)) * (-210),
      Z_Axis => Integer (Step_Per_Millimeter (Z_Axis)) * (-001));
   ...
   Segment_Buffer_Size : constant := 5;
end Settings;
